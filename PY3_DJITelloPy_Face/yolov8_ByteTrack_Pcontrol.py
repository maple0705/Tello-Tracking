...
class FrontEnd(object):
    ...
    def run(self):
        ...
        while not should_stop:
            ...
            for result in results:
                ...
                if rect_ROI is not None:
                    ...
                    if self.auto_pilot:
                        ...
                        error_h = H / 2 - height    # target(half of screen height) - output
                        print(error_h)
                        ...
                        self.yaw_velocity        = int(np.clip(-1 * int(gain_yaw * error_x), -100, 100))
            ...
            center_x = W // 2
            center_y = H // 2
            x1 = center_x - deadband // 2
            x2 = center_x + deadband // 2
            y1 = center_y - deadband // 2
            y2 = center_y + deadband // 2
            for i in range(0, 15, 2):
                dx1 = int(i / 15 * (x2 - x1))
                dx2 = int((i + 1) / 15 * (x2 - x1))
                dy1 = int(i / 15 * (y2 - y1))
                dy2 = int((i + 1) / 15 * (y2 - y1))
                cv2.line(frame, (x1 + dx1, y1), (x1 + dx2, y1), (0, 127, 0), 2)
                cv2.line(frame, (x1 + dx1, y2), (x1 + dx2, y2), (0, 127, 0), 2)
                cv2.line(frame, (x1, y1 + dy1), (x1, y1 + dy2), (0, 127, 0), 2)
                cv2.line(frame, (x2, y1 + dy1), (x2, y1 + dy2), (0, 127, 0), 2)
