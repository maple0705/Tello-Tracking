...
# copy from:
# https://qiita.com/hsgucci/items/5d4e4de933a14820795b
...
def main():
    # カスケード分類器の初期化
    cascPath = os.getcwd() + '/PY3_Tello_Videop_Face/haarcascade_frontalface_alt.xml'    # 分類器データはローカルに置いた物を使う
    ...
    flag_autopilot = 0          # 自動制御をON/OFFするフラグ

    time.sleep(0.5)     # 通信が安定するまでちょっと待つ

    time.sleep(3)     # 通信が安定するまでちょっと待つ
    ...
    try:
        while True:
            ...
            if key == 27:                   # k が27(ESC)だったらwhileループを脱出，プログラム終了
                ...
                flag_autopilot = 1                    # フィードバック制御ON
            elif key == ord('2'):
                flag_autopilot = 0                    # フィードバック制御OFF
                drone.send_command('rc 0 0 0 0')    # ラジコン指令をゼロに
            ...
            if current_time - pre_time > 5.0 :  # 前回時刻から5秒以上経過しているか？
                ...
                battery = drone.send_command('battery?')
